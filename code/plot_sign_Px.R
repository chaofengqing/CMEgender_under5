...
for (year in years.select) {
  for (age in ages.a) {
    Px.cqt <- res[[paste0("Px" ,age, ".cqt")]]
    
    pdf(paste0(Pxplot.dir, "Px", age, "_", runname, "_", round(year), ".pdf"),
        height = 7, width = 10)
    ...
    PlotCIsegments(
      ...
      countryName.c = InternalMakeCountryNamesShort(name.c), data.cqt = Px.cqt,
      cutoff = cutoffP, order = 1, cex = 1.5, xlab = paste0("Px", age),
    ...
    PlotCIsegments(
      Xaxis = TRUE, if.plotall=FALSE, select.t = (year == years.t),
      countryName.c = InternalMakeCountryNamesShort(name.c), data.cqt = Px.cqt,
      cutoff = cutoffP, order = 2, xlab = paste0("Px", age), cex = 1.5,
    ...
    ########################
    ## plot all countries ##
    pdf(paste0(Pxplot.dir, "Px", age, "_", runname, "_allcountry_",
...
#################################################################
## countries are included only if Px sign. Different from 1 AND
...
for (year in years.select) {
  for (age in ages.a) {
    ...
    for (pick in 1:2) {
      ...
      pdf(paste0(Pxplot.dir, "Px", age, "_", runname, "_", round(year),
      ...
      PlotCIsegments(
        ...
        select.c = (Q.ct[, t] >= q.limit[pick]), select.t = (year == years.t),
        countryName.c = InternalMakeCountryNamesShort(name.c),
        cutoff = cutoffP, cex = 1.5, order = 2, xlab = paste0("Px", age),
        main = paste0("Male disadvantage (", round(year), ")"),
        colinfo = c("slategray1", "mediumblue"), lwd.main = 4, lwd.cutoff = 2
      )
      dev.off()
...
#################################################################
## countries are included only if Px sign.
...
for (year in years.select) {
  ...
  for (age in ages.a) {
    ...
    PlotCIsegments(
      Xaxis = TRUE, if.plotall = FALSE, data.cqt = Px.cqt,
      select.c = (Px.cqt[, 2, select.t] < 1 * (100 - Ppercent) / 100),
      select.t = (year == years.t),
      countryName.c = InternalMakeCountryNamesShort(name.c),
      cutoff = cutoffP, cex = 1.5, order = 1, xlab = paste0("Px", age),
      main = paste0("Female disadvantage (", round(year), ")"),
      colinfo = c("lightpink", "maroon2"), lwd.main = 4, lwd.cutoff = 2
    )
    ## above cutoff ##
    PlotCIsegments(
      Xaxis = TRUE, if.plotall = FALSE, data.cqt = Px.cqt,
      select.c = (Px.cqt[, 2, t] > 1 * (100 + Ppercent) / 100),
      select.t = (year == years.t),
      countryName.c = InternalMakeCountryNamesShort(name.c),
      cutoff = cutoffP, cex = 1.5, order = 2, xlab = paste0("Px", age),
      main = paste0("Male disadvantage (", round(year), ")"),
